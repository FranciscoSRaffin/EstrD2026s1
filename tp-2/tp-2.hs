...
-- 2.c
repetir :: Int -> a -> [a]
repetir x y = if (x > 1) then a:(repetir (x-1) y) else x:y
...
promedioEdad ps = (cantidadDeElementos ps) / (sumatoriaDeEdades ps)
...
leGanaA (P Agua _) (P Fuego _) = True
leGanaA (P Fuego _) (P Planta _) = True
leGanaA (P Planta _) (P Agua _) = True
leGanaA _ _ = False

-- B
cantPokemonesDeEn :: TipoDePokemon -> [Pokemon] -> Int
cantPokemonesDeEn _ []   = 0
cantPokemonesDeEn t p:ps = if (sonMismoTipo (tipo p) t)
                                then 1 + sonMismoTipo ps
                                else sonMismoTipo ps

-- c
pokemonesDeTipo :: [Pokemon] -> TipoDePokemon -> [Pokemon]
pokemonesDeTipo []   _ = []
pokemonesDeTipo p:ps t = if (sonMismoTipo (tipo p) t)
                           then p:(pokemonesDeTipo ps)
                           else (pokemonesDeTipo ps)
...
contenidoEn :: [TipoDePokemon] -> [TipoDePokemon] -> Boolean
contenidoEn []     _   = True
contenidoEn p1:ps1 ps2 = (pertenece p1 ps1) && contenidoEn ps1 ps2

pertenece :: Pokemon -> [Pokemon]
pertenece _ []     = False
pertenece p p1:ps1 = (sonMismoTipo p p1) || (pertenece p ps1)
...
losDevSenior (ConsEmpresa rs) = losDevSeniorRoles rs

losDevSeniorRoles :: [Rol] -> [Proyecto]
losDevSeniorRoles []   = []
losDevSeniorRoles r:rs = if ((esDev r) && (esRolSr r))
                            then r:(losDevSeniorRoles rs)
...
esSenioritySr :: Seniority -> Boolean
